...
live_loop :slices do
  ...
  with_fx :slicer do
    64.times do
      ...
      sample :bass_trance_c, amp: 1.1, lpf: 80, slice: range(1,8,1).choose
      sleep 0.25
    end
  end
end
...
live_loop :flanges do
  ...
  with_fx :flanger do
    64.times do
      tick
      synth :chipbass, amp: 0.9, note: (scale :c3, :minor).choose, release: 0.25
      synth :fm, note: (chord :c3, :minor).choose, sustain: 4 if (spread 1,16).look
      sleep 0.25
    end
  end
end
